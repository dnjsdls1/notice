...
const now = new Date(); // 현재 시스템 시간 생성
...
function noticeCreateRender(notice, idx, boolean){
    ...
    if(boolean === false){
        ...
        textSpan3.textContent = notice.id; // 추후 공지사항 등록시 아이디 추가하여 바꾸어야함
...
function noticeDetailClickPage(seq){
    ...
    const index = noticeList.findIndex(n => n.id === authData.id);
    if(index === -1){
        noticeTitle.disabled = true;
        noticeArea.disabled = true;
...
function noticeInsert(){
    ...
    const month = String(now.getMonth() + 1).padStart(2, '0'); // 7월을 '07'로 변환
    const day = String(now.getDate()).padStart(2, '0');      // 2일을 '02'로 변환
